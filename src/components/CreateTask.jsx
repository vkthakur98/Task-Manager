...
const CreateTask = () => {
  ...
  return (
    ...
    <div className='p-4 flex justify-between align-middle font-sans text-[20px]'>
        ...
        <h2 className='font-bold text-white'>Create New Task</h2>
        <span></span>        
    </div>
    <div>
      <div className='p-4 font-sans'><br></br>
      <p className='text-[20px] font-bold text-white'>Task Title</p>
      <input value={Title} onChange={(e)=>{setTitle(e.target.value)}}  type="text" className='p-4 rounded-[20px] border w-[90vw] border-gray-200 bg-[#262626] mt-4 text-white' placeholder='Coding' />
      ...
      <input type='date' onChange={(e)=>{setDate(e.target.value)}} className='p-4 rounded-[20px] border w-[90vw] border-gray-200 bg-[#262626] mt-4 text-white ' ></input>
      <div className='flex justify-between align-middle'>
      <div>
      <p className='text-[18px] font-bold text-white mt-4'>Start Time</p>
      <input type='time' onChange={(e)=>{setStartTime(e.target.value)}} className='p-2 rounded-[17px] border w-[40vw] border-gray-200 bg-[#262626] text-white mt-4' ></input>
      </div>
      <div>
      <p className='text-[18px] font-bold text-white mt-4'>End Time</p>
      <input type='time' onChange={(e)=>{setEndTime(e.target.value)}} className='p-2 rounded-[17px] border w-[40vw] border-gray-200 bg-[#262626] text-white mt-4' ></input>
      </div>
      </div> 
      <p className='text-[20px] font-bold text-white mt-4'>Description</p>
      <textarea  onChange={(e)=>{setDescription(e.target.value)}}  className='p-4 rounded-[20px] border w-[90vw] text-white bg-[#262626] border-gray-200 mt-4' placeholder='type some description about this particular task'></textarea>
